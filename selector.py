...
from constants import (RECTANGLE_LEN, RECTANGLE_WID, SQUARE_DEFAULT_LEN,
                       REC_TO_ARROW_GAP, MOVE)
...
class Selector():
    def __init__(self, screen):
        ...
        self.rectangle.shapesize(stretch_len=RECTANGLE_LEN/SQUARE_DEFAULT_LEN,
                                 stretch_wid=RECTANGLE_WID/SQUARE_DEFAULT_LEN,
                                 outline=2)
        ...
        self.horizontal_pos = [
            [(0, -MOVE),
             (- self.rectangle_len/2 - REC_TO_ARROW_GAP - 25,  - MOVE),
             (self.rectangle_len/2 + REC_TO_ARROW_GAP + 25, -MOVE)],
            [(0, 0),
             (- self.rectangle_len/2 - REC_TO_ARROW_GAP - 25, 0),
             (self.rectangle_len/2 + REC_TO_ARROW_GAP + 25, 0)],
            [(0, MOVE),
             (- self.rectangle_len/2 - REC_TO_ARROW_GAP - 25, MOVE),
             (self.rectangle_len/2 + REC_TO_ARROW_GAP + 25, MOVE)]
        ...
        self.vertical_pos = [
            [(MOVE, 0),
             (MOVE, self.rectangle_len/2 + REC_TO_ARROW_GAP + 25),
             (MOVE, -self.rectangle_len/2 - REC_TO_ARROW_GAP - 25)],
            [(0, 0),
             (0, self.rectangle_len/2 + REC_TO_ARROW_GAP + 25),
             (0, -self.rectangle_len/2 - REC_TO_ARROW_GAP - 25)],
            [(-MOVE, 0),
             (-MOVE, self.rectangle_len/2 + REC_TO_ARROW_GAP + 25),
             (-MOVE, -self.rectangle_len/2 - REC_TO_ARROW_GAP - 25)]
        ...
        self.selector_items = [self.rectangle,
                               self.left_select,
                               self.right_select]
        self.selector_pos = 1
    ...
    def rotate(self):
        ...
        for index, item in enumerate(self.selector_items):
            ...
            if item.heading() == 0:
                ...
                # Done using different images, not by changing heading
                if index == 1:
    ...
    def move_up(self):
        ...
        if self.rectangle.pos() == self.horizontal_pos[2][0] or \
           self.rectangle.heading() == 90:
            pass
        else:
            self.selector_pos += 1
    ...
    def move_down(self):
        ...
        if self.rectangle.pos() == self.horizontal_pos[0][0] or \
           self.rectangle.heading() == 90:
            pass
        else:
            self.selector_pos -= 1
    ...
    def move_left(self):
        ...
        if self.rectangle.pos() == self.vertical_pos[2][0] or \
           self.rectangle.heading() == 0:
            pass
        else:
            self.selector_pos += 1
            # Set new position for each item
    ...
    def move_right(self):
        ...
        if self.rectangle.pos() == self.vertical_pos[0][0] or \
           self.rectangle.heading() == 0:
            pass
        else:
            self.selector_pos -= 1
